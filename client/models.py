...
class Client(models.Model):
    ...
    def kind_of_work(self):
        """бизнес или работа на "дядю" """
        if self.job.first():
            return 'default job'
        elif self.client_commercial.first():
    ...
    def clean(self):
        ...
        if 67 > self.get_age or self.get_age <= 21:
            raise ValidationError({'date_of_birth': 'Возраст клиента должен быть от 18 до 67 лет включительно'})
...
class Job(models.Model):
    ...
    def get_work_period(self):
        today = timezone.now().date()
        period = (today.year - self.date_of_employment.year - \
            ((today.month, today.day) <
            (self.date_of_employment.month, self.date_of_employment.day))) * 12
        return period
...
class ClientCommercial(models.Model):
    ...
    def __str__(self):
        return f"{self.client}|{self.company_name}"
